...
if __name__ == "__main__":
    ...
    for epoch in range(20):
        ...
        for i, (x, y) in enumerate(pbar):
            ...
            e_noise = torch.randn_like(x)
            t_en = torch.randint(low=0, high=t_max_steps, size=(B,), dtype=torch.long)
            alpha_bar_t = alpha_prod_tensor[t_en].float().view(B, 1, 1, 1)
            ...
            training_loss += loss
            training_count += 1
            pbar.set_postfix({"loss": f"{(training_loss / training_count):.5f}"})
        ...
        with torch.no_grad():
            for i, (x, y) in enumerate(pbar):
                ...
                validation_count += 1
                pbar.set_postfix({"loss": f"{(validation_loss / validation_count):.5f}"})
    ...
    for d in tqdm(range(num_digits), desc="Generating samples"):
        samples = sample_reverse(model, num_per_digit, d)
