...
rem Builds the gaussian splat demo to WebAssembly and copies the bundle into
rem site/demos/basic-gaussians/ where nginx serves it.
rem
...
set "DEPLOYDIR=%PROJDIR%..\..\site\demos\basic-gaussians"
...
if exist "%BUILDDIR%\BasicGaussians.html" del /q "%BUILDDIR%\BasicGaussians.html"
...
rem Nothing is preloaded into the virtual FS, so there is no .data to copy.
if not exist "%DEPLOYDIR%" mkdir "%DEPLOYDIR%"
copy /y "%BUILDDIR%\BasicGaussians.html" "%DEPLOYDIR%\index.html" >nul || exit /b 1
for %%F in (js wasm) do (
    copy /y "%BUILDDIR%\BasicGaussians.%%F" "%DEPLOYDIR%\" >nul || exit /b 1
)

echo [done] deployed to site\demos\basic-gaussians\
echo        preview: python -m http.server 8899 --directory "%PROJDIR%..\..\site"
echo                 then open http://localhost:8899/project-basic-gaussians.html
